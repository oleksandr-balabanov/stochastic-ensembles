...
def plot_metrics(args):
    ...
    if args.cifar_mode == "CIFAR10":
        ...
        accuracy_loss_CIFARC = serialize.load(get_eval_folder(args) / f"accuracy_loss_CIFAR10C_nn{args.num_nets:02d}.dill")["accuracy_loss_CIFARC"]
        file_name_ac = "accuracy_cifar10c"
        file_name_loss = "loss_cifar10c"
        num_classes = 10

    if args.cifar_mode == "CIFAR100":

        # load and shape the data
        accuracy_loss_CIFARC = serialize.load(get_eval_folder(args) / f"accuracy_loss_CIFAR100C_nn{args.num_nets:02d}.dill")["accuracy_loss_CIFARC"]
        file_name_ac = "accuracy_cifar100c"
        file_name_loss = "loss_cifar100c"
        num_classes = 100



    # load softmax probs and targets
    res_dic = serialize.load(get_eval_folder(args) /  f"performance_metrics_nn{args.num_nets:02d}.dill")

    # compute performance metrics
    calibration_metrics = res_dic["calibration_metrics"]
    entropy_metrics = res_dic["entropy_metrics"]
    ...
    bins = np.arange(0, np.log(num_classes), np.log(num_classes)/9)
    file_name = "entropy_histogram"
    plot_single_entropy(entropy_metrics["entropy"], bins, file_name, args)
    
    # plot accuracy and loss estimates associated with CIFAR10C and CIFAR100C corruptions
    accuracy_mean, accuracy_min_max, accuracy_q_min_max = get_accuracy_metrics_CIFARC(accuracy_loss_CIFARC["accuracy"])
    loss_mean, loss_min_max, loss_q_min_max = get_loss_metrics_CIFARC(accuracy_loss_CIFARC["loss"])

    plot_single_cifarc_loss_accuracy(accuracy_mean, accuracy_min_max,  accuracy_q_min_max, file_name_ac, args) 
    plot_single_cifarc_loss_accuracy(loss_mean, loss_min_max,  loss_q_min_max, file_name_loss, args)
...
def plot_single_calibration_curve(calibration_metrics, file_name, args):
    ...
    plt.savefig(get_eval_folder(args) /  f"img_{file_name}_nn{args.num_nets:02d}.pdf", format='pdf', bbox_inches='tight')



# plot one entropy or mutual info subplot
def plot_single_entropy(data, bins, file_name, args, xtext = 0.28, ytext = 0.73):
...
# plot loss CIFARC 
def plot_single_cifarc_loss_accuracy(res_mean, res_min_max,  q, file_name, args):
    ...
    ax.errorbar(xlist, res_mean, res_min_max, linestyle='None', color = "black", capsize=2, capthick=1, lw=1)
    
    # add rectangle to the plot
    for level in range(5):
        ax.add_patch(Rectangle((level + 1, q_min[level]), 0.5, -q_min[level]+q_max[level],
                 edgecolor = 'black',
                 facecolor = color,
                 fill=True,
                 lw=1,zorder=2))
...
def get_accuracy_metrics_CIFARC(accuracy):
...
def get_loss_metrics_CIFARC(loss):
