...
class DropOutConv2d(nn.Module):  
    def __init__(self, in_planes, planes, kernel_size=3, stride=1, padding=0, bias=True, drop_rate=0):
...
class NPDropOutConv2d(nn.Module):  
    def __init__(self, in_planes, planes, kernel_size=3, stride=1, padding=0, bias=True):
        ...
        for swap in range(2):
            self.conv.append(nn.Conv2d(in_planes, planes, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias))
...
class DropConnectLinear(nn.Module):  
    ...
    def forward(self, x):
        ...
        # dropconnect weight
        w = torch.nn.functional.dropout(self.linear.weight, p=self.drop_rate, training=self.training)
...
class DropConnectConv2d(nn.Module):  
    ...
    def forward(self, x):
        ...
        b = None
        if self.bias == True:
            b = torch.nn.functional.dropout(self.conv.bias, p=self.drop_rate, training=self.training)
