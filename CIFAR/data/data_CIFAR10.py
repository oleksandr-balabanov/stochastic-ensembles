...
def get_cifar10(data_dir):
    ...
    for i in range(5):
        data_batch = unpickle(os.path.join(data_dir, 'CIFAR/cifar-10-batches-py/data_batch_' + str(i + 1)))
...
def create_datasets(train_data, test_data, num_train, shuffle = True, do_augmentation_train = False, do_augmentation_test = False):
    ...
    # datasets
    train_dataset = preprocessed_CIFAR10_dataset(train_images, train_labels, test = False, do_augmentation = do_augmentation_train)
    valid_dataset = preprocessed_CIFAR10_dataset(valid_images, valid_labels, test = False, do_augmentation = do_augmentation_train)
    test_dataset = preprocessed_CIFAR10_dataset(test_images, test_labels, test = True, do_augmentation = do_augmentation_test)
